...
| 32 | computed 16-int polygon from `n1` then `ta.a(poly,124,wfb)` | Exact recovered construction from bytecode 5062–5456. With `n=n1`, generated array: `{ -n,n,-n,-n,-n,-n,n,-n,n,n,n,n,n,n,-n,n }`; `wfb` origin `(-n,0)`; `wfb.a(105,15)`; `wfb.a=2`. |
| 33 | computed 16-int polygon from `fnb.b`/`fnb.a` then `ta.a(poly,114,wfb)` | **Corrected slot attribution.** Bytecode 5850–6095 builds this polygon and explicitly stores it into `hab.g[34]` only after the construction completes; therefore the generated polygon immediately preceding that store is slot 33. Let `b=fnb.b=5120`, `h=fnb.a=4434`, `r=b/2=2560`. Starting `(x,y)=(0,-h+r)`, exact array: `{0,-h+r,r,r,3*r,3*r,3*r,3*r,r,3*r,r,r+h,h,r-h,r-h,h-2*r}`; `wfb` origin `(-r,0)`; `wfb.a(114,15)`; `wfb.a=2`. |
...
| 37 | computed 12-int polygon from `n1,n2` then `ta.a(poly,127,wfb)` | Exact recovered construction from bytecode 5563–5843. Starting `x=0`, `y=-2*n2-n1`, generated array: `{0,-2*n2-n1,n1,-n1,n1,n1,0,n1+2*n2,-n1,n1,-n1,-n1}`; `new wfb(...,-n1,0)`; `wfb.a(118,15)`; the surrounding preceding construction explicitly sets `a=3` on slot 37 before the later generated component is assigned to slot 33. `B=0`, `q=0`. |
...
**Forensic note:** the raw bytecode contains several adjacent generated constructions. Slot identity must be taken from the explicit `hab.g[index] = wfb` stores, not inferred solely from bytecode-range labels. Do not replace these expressions with hand-tuned static geometry.
